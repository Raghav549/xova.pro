...
describe('StreamingMessageParser', () => {
  ...
  describe('no artifacts', () => {
    ...
    ])('should correctly parse chunks and strip out xova artifacts (%#)', (input, expected) => {
  ...
  describe('invalid or incomplete artifacts', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      ...
      ['Foo bar <bol', 'Foo bar '],
      ['Foo bar <xova', 'Foo bar '],
      ['Foo bar <xovaa', 'Foo bar <xovaa'],
      ['Foo bar <xovaA', 'Foo bar '],
      ['Foo bar <xovaArtifacs></xovaArtifact>', 'Foo bar <xovaArtifacs></xovaArtifact>'],
      ['Before <oltArtfiact>foo</xovaArtifact> After', 'Before <oltArtfiact>foo</xovaArtifact> After'],
      ['Before <xovaArtifactt>foo</xovaArtifact> After', 'Before <xovaArtifactt>foo</xovaArtifact> After'],
    ])('should correctly parse chunks and strip out xova artifacts (%#)', (input, expected) => {
      runTest(input, expected);
    });
  });
  ...
  describe('valid artifacts without actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Some text before <xovaArtifact title="Some title" id="artifact_1">foo bar</xovaArtifact> Some more text',
        {
      ...
      [
        ['Some text before <xovaArti', 'fact', ' title="Some title" id="artifact_1">foo</xovaArtifact> Some more text'],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
      ...
      [
        [
          'Some text before <xovaArti',
          'fac',
          't title="Some title" id="artifact_1"',
          ' ',
          '>',
          'foo</xovaArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        'Before <xovaArtifact title="Some title" id="artifact_1">foo</xovaArtifact> After',
        {
          output: 'Before  After',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
  ...
  describe('valid artifacts with actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Before <xovaArtifact title="Some title" id="artifact_1"><xovaAction type="shell">npm install</xovaAction></xovaArtifact> After',
        {
          output: 'Before  After',
      ...
      [
        'Before <xovaArtifact title="Some title" id="artifact_1"><xovaAction type="shell">npm install</xovaAction><xovaAction type="file" filePath="index.js">some content</xovaAction></xovaArtifact> After',
        {
          output: 'Before  After',
